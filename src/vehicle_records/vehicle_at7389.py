...
def page_AT7389():
    ...
    editable_grid(bus_number='AT7389')


def expense_AT7389():

    background()
    st.header("Expense page AT7389 ", text_alignment='center')
    col1, col2 = st.columns(2)
    ...
    if st.session_state['expense_tab'] == 'vehicle':
        expenses(bus_number='AT7389')
    elif st.session_state['expense_tab'] == 'driver':
        driver_salary(bus_number='AT7389')
    elif st.session_state['expense_tab'] == 'diesel':
        diesel_view(bus_number='AT7389')
