    ...
    for _, row in df.iterrows():
        ...
        if existing.data:
            ...
            merged = {
                ...
                "scheduled_km":    keep(new_data["scheduled_km"],   old.get("scheduled_km"),   [0, None]),
                "actual_km":       keep(new_data["actual_km"],      old.get("actual_km"),      [0, None]),
                "diesel":          keep(new_data["diesel"],         old.get("diesel"),          [0.0, 0, None]),
                "diesel_km":       keep(new_data["diesel_km"],      old.get("diesel_km"),       [0, None]),
                "income":          keep(new_data["income"],         old.get("income"),          [0, None]),
            }
